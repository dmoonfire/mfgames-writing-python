...
# Internal Imports
from mfgames.convert import setup_convert

#
# Setup
#

def setup_creole_to_docbook(subparsers):
    """Creates a parser for converting creole to docbook."""

    # Logging
    log = logging.getLogger('docbook')

    # Create a sub-parser for 'docbook'.
    docbook_parser = subparsers.add_parser(
        'docbook',
        help='Converts creole files into docbook.')
    setup_convert(docbook_parser)
    docbook_parser.set_defaults(convert_function=convert_creole_to_docbook)
    docbook_parser.set_defaults(convert_extension=get_docbook_extension)
...
def convert_creole_to_docbook(args, input_filename, output_filename):
    """
    ...
    contents = wrap_sections(contents, "section", 'h5',
        [ 'h4', 'h3', 'h2', 'h1' ])
    contents = wrap_sections(contents, "section", 'h4', [ 'h3', 'h2', 'h1' ])
    contents = wrap_sections(contents, "section", 'h3', [ 'h2', 'h1' ])
    contents = wrap_sections(contents, "section", 'h2', [ 'h1' ])
    contents = wrap_sections(contents, "article", 'h1', [ ])
...
def wrap_sections(contents, element, heading, outer_headings):
    """
